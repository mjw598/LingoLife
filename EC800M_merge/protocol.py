...
class WebSocketClient(object):
    ...
    def connect(self):
        ...
        try:
            ...
            logger.error("{} websocket connect failed: {}".format(self, repr(e)))
            return None
        ...
        try:
            ...
            logger.info("{} websocket connected".format(self))
            return __client__

    def connect_with_retry(self, retries=5, base_delay_ms=1000, max_delay_ms=15000):
        """Connect with bounded backoff for boot-time network jitter."""
        delay = base_delay_ms
        for attempt in range(retries):
            client = self.connect()
            if client is not None:
                return client
            logger.info("{} websocket retry {}/{} after {} ms".format(
                self, attempt + 1, retries, delay))
            utime.sleep_ms(delay)
            delay = min(delay * 2, max_delay_ms)
        raise RuntimeError("{} websocket connect retry exhausted".format(self))
    ...
    def recv(self):
        ...
        if type(data) == str:
            ...
            if text_value != self.__last_text_value and text_value is not None:
                self.__last_text_value = text_value
    ...
    def hello(self):
        ...
        try:
            ...
            logger.error("hello failed: {}".format(repr(e)))
            return None
    ...
    def start_assistant_greeting(self, session_id=""):
        # Use an explicit new-session opener instead of a bare wake word.
        # A bare wake word can be interpreted as an exit/closing intent after
        # the previous assistant session has just ended.
        with self.__resp_helper:
            self.send(
                JsonMessage(
                    {
                        ...
                        "text": (
                            ...
                            "Open with one short sentence about what you can do. "
                            "For this whole session, keep every reply to one short sentence. "
                            "Do not mention your name, identity, model, or brand. "
                            "Do not say goodbye."
                        )
                    }
                ).to_bytes()
    ...
    def ping(self):
        self.send(JsonMessage({"type": "ping"}).to_bytes())
