...
def record_start():
    ...
    global _pcm, _rec_buf, _rec_len, _recording
    if _PCM_CLS is None:
...
def do_asr_and_send():
    ...
    try:
        ...
        t2 = utime.ticks_ms()
        text = asr_recognize(TMP_REC)
        t3 = utime.ticks_ms()
        _uart_log("timing stop=%dms trim=%dms(%d->%d) asr=%dms" % (
            utime.ticks_diff(t1, t0),
            utime.ticks_diff(t2, t1),
            sz, sz_after,
            utime.ticks_diff(t3, t2)))
        if text:
            line = "USER:%s\n" % text
